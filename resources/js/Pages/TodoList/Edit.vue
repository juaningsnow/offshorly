...
export default {
  ...
  components: {
    BreezeAuthenticatedLayout,Head,useForm
  },
  data() {
    return {
          form: useForm({
  ...
  methods: {
    submit(){
      this.form.patch(`/todo-lists/${this.todoList.id}`);
    }
  },
  mounted(){
    if(this.todoList){
      this.form.title = this.todoList.title;
      this.form.description = this.todoList.description;
      this.form.date = this.todoList.date;
    }
  }
}
